...
import { Search, Plus, Target, Flag, Sun, Cloud, Moon, Star, Check } from 'lucide-react';
...
const slotConfig: Record<TimeSlot, { icon: typeof Sun; label: string; color: string }> = {
  morning: { icon: Sun, label: 'Morning', color: 'text-amber-600 dark:text-amber-400' },
  afternoon: { icon: Cloud, label: 'Afternoon', color: 'text-blue-600 dark:text-blue-400' },
  evening: { icon: Moon, label: 'Evening', color: 'text-purple-600 dark:text-purple-400' },
  night: { icon: Star, label: 'Night', color: 'text-indigo-600 dark:text-indigo-400' }
};
    ...
    <BottomSheet
      ...
      title={
        <div className="flex items-center gap-2">
          ...
          <span>{t('planner.addToSlot').replace('{slot}', t(`planner.${targetSlot}`))}</span>
        </div>
      }
      height="full"
    >
                      ...
                      >
                        ...
                        >
                          {wasJustAssigned ? (
                            ...
                            <Plus className={`w-5 h-5 ${isAssigning ? 'animate-pulse' : ''}`} />
                          )}
                        </button>
                      </div>
                    );
