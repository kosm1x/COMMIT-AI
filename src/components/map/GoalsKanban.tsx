  ...
  const loadGoals = async () => {
    ...
    const [goalsResult, objectivesResult, tasksResult] = await Promise.all([
      supabase
        .from("goals")
        .select(
          'id, vision_id, title, description, status, target_date, "order", last_edited_at, visions(id, title)',
        )
        .eq("user_id", user!.id)
    ...
    if (goalsResult.data) {
      ...
      if (!hasAppliedSort.current) {
        ...
        loadedGoals = sortGoals(
          loadedGoals as unknown as import("../../components/objectives/types").Goal[],
        ) as unknown as Goal[];
      }
  ...
  const handleDrop = async (
    ...
    if (sameStatus && targetGoalId) {
      ...
      const statusGoals = goals
        .filter((g) => g.status === targetStatus)
      ...
      const newOrder =
        statusGoals.length > 0
          ? Math.max(...statusGoals.map((g) => g.order)) + 1
          : 0;
  ...
  const getGoalsByStatus = (status: string) => {
    ...
    if (hasSelection) {
      filteredGoals = filteredGoals.filter((goal) =>
        isInSelectedFamily("goal", goal.id),
      );
    }
    ...
    if (selectedVisionId) {
      filteredGoals = filteredGoals.filter(
        (goal) => goal.vision_id === selectedVisionId,
      );
    }
                ...
                return (
                  <div
                    ...
                    className={`bg-white dark:bg-white/10 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-white/10 cursor-move hover:shadow-md transition-all ${
                      ...
                      isHighlighted
                        ? "ring-4 ring-blue-400 ring-offset-2 dark:ring-offset-gray-900 animate-pulse shadow-lg shadow-blue-500/30"
                        : ""
                    }`}
                  >
