...
export default function VisionsKanban({
  selectedVisionId,
  selectedGoalId,
  selectedObjectiveId,
  selectedTaskId,
  ...
  const loadVisions = async () => {
    ...
    if (visionsResult.data) {
      setVisions(visionsResult.data as Vision[]);
    }
    if (goalsResult.data) {
  ...
  const handleDrop = async (
    ...
    if (sameStatus && targetVisionId) {
      ...
      const statusVisions = visions
        .filter((v) => v.status === targetStatus)
        .sort((a, b) => a.order - b.order);
      const targetIndex = statusVisions.findIndex(
        (v) => v.id === targetVisionId,
      );
      ...
      const statusVisions = visions
        ...
        .sort((a, b) => a.order - b.order);
      const newOrder =
        statusVisions.length > 0
          ? Math.max(...statusVisions.map((v) => v.order)) + 1
          : 0;
  ...
  const getVisionsByStatus = (status: string) => {
    ...
    if (hasSelection) {
      filteredVisions = filteredVisions.filter((vision) =>
        isInSelectedFamily("vision", vision.id),
      );
    }
                ...
                return (
                  <div
                    ...
                    className={`bg-white dark:bg-white/10 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-white/10 cursor-move hover:shadow-md transition-all ${
                      ...
                      isHighlighted
                        ? "ring-4 ring-amber-400 ring-offset-2 dark:ring-offset-gray-900 animate-pulse shadow-lg shadow-amber-500/30"
                        : ""
                    }`}
                  >
