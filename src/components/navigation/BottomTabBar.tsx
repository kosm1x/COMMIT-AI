...
export default function BottomTabBar({ onQuickAdd }: BottomTabBarProps) {
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 lg:hidden z-40 backdrop-blur-sm"
    >
      <div className="flex items-center justify-around px-2 py-2 safe-area-inset-bottom bg-white/95 dark:bg-gray-900/95">
        {mobileNavItems.map((item, index) => {
          ...
          const isActive =
            ...
            (item.path === "/objectives" &&
              ["/vision", "/goals", "/tasks"].includes(location.pathname));
