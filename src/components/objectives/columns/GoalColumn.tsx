...
export function GoalColumn({
  ...
  const totalCount =
    displayVisionGoals.length +
    (showOrphaned ? visibleOrphanedGoals.length : 0);
    ...
    return (
      <div key={goal.id} ref={isEditing ? editingCardRef : null}>
        <GoalCard
          ...
          onTitleClick={(e) =>
            onTitleClick("goal", goal.title, goal.description || "", e)
          }
          onConvertToVision={
            onConvertToVision ? () => onConvertToVision(goal) : undefined
          }
          onConvertToObjective={
            onConvertToObjective
              ? (targetGoalId) => onConvertToObjective(goal, targetGoalId)
              : undefined
          }
