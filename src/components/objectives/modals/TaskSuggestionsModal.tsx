...
interface TaskSuggestionsModalProps {
  ...
  error: string | null;
  onClose: () => void;
  onCreateTasks: (selected: SuggestedTask[]) => Promise<void>;
  onRegenerate: () => void;
}
...
export default function TaskSuggestionsModal({
  ...
  const [editedSuggestions, setEditedSuggestions] = useState<SuggestedTask[]>(suggestions);
  ...
  const handleEdit = (index: number, field: keyof SuggestedTask, value: string) => {
  ...
  const handleCreate = async () => {
    const selectedTasks = Array.from(selected)
      .sort((a, b) => a - b)
      ...
      <div className="glass-strong rounded-2xl max-w-3xl w-full max-h-[80vh] flex flex-col shadow-2xl border border-white/20 dark:border-white/10 animate-scale-in">
        ...
        <div className="flex items-start justify-between p-6 border-b border-white/10">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-text-primary">AI-Suggested Tasks</h2>
              <p className="text-sm text-text-secondary mt-1">
                For objective: <span className="font-medium">{objectiveTitle}</span>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {loading && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-text-secondary">Analyzing objective and generating tasks...</p>
            </div>
          )}
          ...
          {!loading &&
            !error &&
            editedSuggestions.map((task, index) => (
              <div
                key={index}
                    ...
                    <div
                      className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                        selected.has(index)
                          ...
                          : 'border-border-secondary hover:border-purple-400'
                      }`}
                    >
                      ...
                      <>
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <button
                            ...
                            className="text-purple-600 hover:text-purple-700 dark:text-purple-400 p-1"
                            title="Edit task"
                          >
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-xs text-text-tertiary">{task.reasoning}</span>
                        </div>
                      </>
                    )}
                  </div>
        ...
        {!loading && !error && suggestions.length > 0 && (
          <div className="flex items-center justify-between p-6 border-t border-white/10">
            <div className="flex items-center gap-3">
              <button
                onClick={onRegenerate}
                className="text-sm text-text-secondary hover:text-purple-600 dark:hover:text-purple-400 transition-colors"
              >
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="btn-primary bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {creating ? (
                  <>
