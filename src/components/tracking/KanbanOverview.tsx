...
export default function KanbanOverview() {
  ...
  const handleItemClick = (item: Item) => {
    navigate('/boards', { state: { selectItem: { id: item.id, type: item.type } } });
  };
  ...
  const loadAllItems = async () => {
    ...
    try {
      const [visionsResult, goalsResult, objectivesResult, tasksResult] = await Promise.all([
        supabase.from('visions').select('id, title, status').eq('user_id', user!.id),
        supabase.from('goals').select('id, title, status').eq('user_id', user!.id),
        supabase.from('objectives').select('id, title, status').eq('user_id', user!.id),
        supabase.from('tasks').select('id, title, status, is_recurring').eq('user_id', user!.id),
      ]);

      const items: Item[] = [
        ...(visionsResult.data || []).map(v => ({ ...v, type: 'vision' as const })),
        ...(goalsResult.data || []).map(g => ({ ...g, type: 'goal' as const })),
        ...(objectivesResult.data || []).map(o => ({ ...o, type: 'objective' as const })),
        ...(tasksResult.data || []).map(t => ({ ...t, type: 'task' as const })),
      ];
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {Object.entries(TYPE_CONFIG).map(([type, config]) => {
          ...
          return (
            <button
              key={type}
              onClick={() => handleTypeClick(type as 'vision' | 'goal' | 'objective' | 'task')}
                ...
                <div className="space-y-1.5 max-h-[200px] overflow-y-auto custom-scrollbar pr-1">
                  {items.slice(0, 10).map((item) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`${config.color} p-1.5 rounded border border-white/40 dark:border-white/10 flex-shrink-0`}>
