...
export default function WeeklyView({ selectedDate }: WeeklyViewProps) {
  ...
  const loadWeeklyData = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        totalCompleted += completedOnDay.length + recurringCompletedCount;
  ...
  const weekPercentage = weeklyTotals.total > 0
      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-3">
          {weekStats.map((day, index) => {
            ...
            const barHeight = day.total > 0 ? Math.max((day.total / maxDailyTotal) * 100, 20) : 20;
                ...
                <div className="relative h-24 bg-white/30 dark:bg-white/5 rounded-lg flex items-end justify-center overflow-hidden border border-white/40 dark:border-white/10">
                  <div
                    className={`w-full transition-all duration-300 ${
                      dayPercentage === 100 ? 'bg-green-500 dark:bg-green-600' : 'bg-blue-500 dark:bg-blue-600'
                    }`}
