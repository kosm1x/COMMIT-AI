  ...
  const signOut = async () => {
    try {
      ...
      if (user) {
        ...
        // Clear all app localStorage keys to prevent leakage between users
        [
          "theme",
          "commit_language",
          "commit_user_preferences",
          "commit_last_page",
          "tracking_dashboard_layout_v4",
          "commit_biometric_enabled",
        ].forEach((k) => localStorage.removeItem(k));

        // Clear user-scoped keys (with dynamic suffixes)
        const userId = user.id;
        Object.keys(localStorage).forEach((key) => {
          if (
            key.startsWith("commit_welcome_modal_") ||
            key.startsWith(`time_spent_${userId}`) ||
            key.startsWith(`last_update_${userId}`) ||
            key.startsWith(`last_activity_${userId}`) ||
            key.startsWith(`is_active_${userId}`)
          ) {
